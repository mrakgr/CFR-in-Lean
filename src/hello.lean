...
def u.template_calc (σ : Policy infoset_sizes) : GameTree infoset_sizes → ℚ
| (GameTree.Terminal _ reward) := reward
| (GameTree.Response id' subnodes) :=
    let σ := σ.val id' in
    @nat.foldl.fin (fun i, rat) (infoset_sizes id') 0 (fun i s, s + σ i * u.template_calc (subnodes i))
...
| p' (GameTree.Response id' subnodes) := 
    if p = p' ∧ id = id' then
        let σ' := σ.val id' in
        @nat.foldl.fin (fun i, rat) (infoset_sizes id') 0 $ fun i s, s + σ' i * u.template_calc σ (subnodes i)
    else 
